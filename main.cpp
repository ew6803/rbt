...
void updateTree(Node* &root, Node* &pt) {

}

void printTree(Node* head, int format) {
  if (head == NULL) 
    return; 
  format += 10;
  printTree(head -> getRight(), format);
  cout << '\n';
  for (int i = 10; i < format; i++)
    cout << " ";
  if (head -> getColor() == 1) {
    cout << "Red: " << head -> getData(); 
  }
  else {
    cout << "Black: " << head -> getData(); 
  }
      printTree(head -> getLeft(), format); 
